...
const QuizCard = ({quiz, onDelete}) => {
    return (
        <div className="group realtive bg-white/80 backdrop-blur-xl border-2 border-slate-200 hover:border-emerald-300 rounded-2xl p-4 transition-all duration-200 hover:shadow-lg hover:shadow-emerald-500/10 flex flex-col justify-between">
            <button
            onClick={(e) => {
                e.stopPropagation();
                onDelete(quiz);
            }}
            className="absolute top-4 right-4 p-2 text-slate-400 hover:text-rose-500 hover:bg-rose-50 rounded-lg transition-all duration-200 opacity-0 group-hover:opacity-200"
            >
            ...
            <div>
                ...
                <p className="text-xs font-medium text-slate-900 mb-1 line-clamp-2">
            ...
            <div className="mt-2 pt-4 border-t border-slate-100">
                {quiz?.userAnswers?.length > 0 ? (
                    ...
                    <Link to={`/quizzes/${quiz._id}`}>
                        <button className="group/btn relative w-full h-11 bg-linear-to-r from-emerald-500 to-teal-500 hover:from-emerald-600 hover:to-teal-600 text-white font-semibold text-sm rounded-xl tranistion-all duration-200 shadow-lg shadow-emerald-500/25 active:scale-95 overflow:hidden">
