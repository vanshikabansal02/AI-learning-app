...
const Flashcard = ({ flashcard, onToggleStar }) => {
  ...
  return <div className="relative w-full h-72" style={{ perspective: '1000px' }}>
    ...
    >
      ...
      >
        {/* Star Button */}
        <div className="flex items-start judtify-between">
          <div className='bg-slate-100 text-[10px] text-slate-600 rounded px-4 py-1'>
            <button
              onClick={(e) => {
                e.stopPropagation();
              ...
              className={`w-9 h-9 rounded-xl flex items-center justify-center transition-all duration-200 ${
                flashcard.isStarred
                  ? 'bg-linear-to-br from-amber-400 to-yellow-500 text-white shadow-l'
                  : 'bg-slate-100 text-slate-400 hover:bg-slate-200 hover:text-amber'
              }`}
            >
              <Star
                className="w-4 h-4"
