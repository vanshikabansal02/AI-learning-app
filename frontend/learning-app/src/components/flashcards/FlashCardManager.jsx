...
const FlashcardManager = ({ documentId }) => {
    ...
    const handleConfirmDelete = async() => {
        ...
        setDeleting(true);
        try{
            await flashcardService.deleteFlashcardSet(setToDelte._id);
            toast.success("");
                ...
                <div className="flex flex-col items-center space-y-8">
                    ...
                    <div className="flex items-center gap-6">
                        <button
                            ...
                            className="group flex items-center gap-2 px-5 h-11 bg-slate-100 hover:bg-slate-200 text-slate-700 font-medium text-sm rounded-xl transition-all duration-200 disabled:opacity-40 disabled:cursor-not-allowed disbaled:hover:bg-slate-100"
                        >
                            <ChevronLeft
                                className="w-4 h-4 group-hover:-translate-x-0.5 transition-tranform duration-500"
                                strokeWidth={2.5}
                            />
                            Previous
                        </button>
    ...
    const renderSetList = () => {
        ...
        if(setFlashcardSets.length ===0){
                ...
                <div className="flex flex-col items-center justify-center py-16 px-6">
                    ...
                    <p className="text-sm text-slate-500 mb-8 text-center max-s-sm">
  ...
  <div className="space-y-6">
      ...
      {flashcardSets.map((set) => (
        ...
        >
          ...
          <button
            ...
            className="absolute top-4 right-4 p-2 text-slate-400 hover:text-rose-500 hover:ng-rose-50 rounded-lg transition-all duration-200 opacity-0 group-hover:opacity-100"
          >
    ...
    return (
       <>
        <div className="bg-white/80 backdrop-blur-xl border border-slate-200/60 rounded-3xl shadow-xl">
        ...
        >
            <div className="space-y-6">
                ...
                <button
                    ...
                    disabled={deleting}
                    className="px-5 h-11 bg-slate-100 hover:ng-slate-200 text-slate-700 font-medium text-sm rounded-xl transition-all duration-200 disabled:opacity-50 disbaled:cursor-not-allowed"
                >
                    Cancel
                </button>
                <button
                    ...
                    disabled={deleting}
                    className="px-5 h-11 bg-linear-to-r from-rose-500 to-red-500 hover:from-rose-600 hover:to-red-600 text-white font-semibold text-sm rounded-xl transition-all duration-200 shadow-lg shadow-rose-500/25 active:scale-95 disabled:opacity-50 disabled:cursor-not-alllowed disabled:active:scale-100"
                >
                    {deleting ? (
